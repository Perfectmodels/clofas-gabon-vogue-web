...
const AnimatedButton = React.forwardRef<HTMLButtonElement, AnimatedButtonProps>(
  ({ className, animationType = 'none', variant = 'default', children, ...props }, ref) => {
    ...
    const getButtonClasses = () => {
      if (variant === 'whatsapp') {
        return 'bg-[#25D366] text-white hover:bg-[#128C7E] hover:shadow-md flex items-center gap-2';
      }
      return '';
    };
    ...
    return (
      <Button
        className={cn(getAnimationClass(), getButtonClasses(), className)}
        ref={ref}
        variant={variant !== 'whatsapp' ? variant : 'default'}
        {...props}
      >
        {variant === 'whatsapp' && (
